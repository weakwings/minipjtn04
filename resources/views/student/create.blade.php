...
<body class="flex flex-col items-center justify-center min-h-screen bg-gray-100">
    <div class="w-full max-w-3xl p-10 bg-white rounded shadow-md">
        <h1 class="mb-8 text-3xl font-bold text-center">Create Student</h1>
        <form action="{{ route('student.store') }}" method="POST" class="space-y-8">
            @csrf
            <div class="flex flex-wrap -mx-2">
                ...
                <div class="w-full px-2 pt-4">
                    ...
                    <select name="courses[]" id="courses[]" multiple
                        class="block w-full h-auto py-2 mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        @foreach ($courses as $course)
                            <option value="{{ $course->id }}">{{ $course->matrix_course }}</option>
                        @endforeach
                    </select>
                    <div class="pt-2">
                        <p>Select more than one course using ctrl.</p>
                    </div>
                </div>
